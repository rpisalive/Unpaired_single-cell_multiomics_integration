...
fig3_dir <- file.path(base_dir, "sc_vs_scot+/MOFA_complied/Figure3")
outdir   <- file.path(base_dir, "sc_vs_scot+/MOFA_complied/Figure4")
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

matched_pairs_file <- file.path(fig3_dir, "Figure3_matched_factor_pairs.csv")
...
make_combined_dotplot <- function(df_view) {
  ...
  factor_pos <- data.frame(
    Factor = factor(paste0("Factor", sort(unique(df_view$FactorNumber))),
  ...
  df_view <- df_view %>%
    ...
    mutate(
      x = case_when(
        Model == "Reference" ~ x_ref,
        Model == "SCOT+-aligned" ~ x_scot
      )
    ) %>%
    filter(!is.na(x))
...
ref_enrich_list <- run_model_enrichment_all(
  model = m_ref_gsea,
  model_label = "Reference",
  max_pathways = max_pathways
)

scot_enrich_list <- run_model_enrichment_all(
  model = m_scot_gsea,
  model_label = "SCOT+-aligned",
  max_pathways = max_pathways
)

# Combine by view
df_rna <- bind_rows(
  ref_enrich_list[["Transcriptomics"]],
  scot_enrich_list[["Transcriptomics"]]
)

df_prot <- bind_rows(
  ref_enrich_list[["Proteomics"]],
  scot_enrich_list[["Proteomics"]]
)

# Save underlying data
write.csv(df_rna,  file.path(outdir, "Figure4A_Transcriptomics_combined_dotplot_data.csv"), row.names = FALSE)
write.csv(df_prot, file.path(outdir, "Figure4B_Proteomics_combined_dotplot_data.csv"), row.names = FALSE)

# -----------------------------
# Figure 4A and 4B dot plots
# -----------------------------
p4A <- make_combined_dotplot(df_rna)
p4B <- make_combined_dotplot(df_prot)
...
matched_pairs <- matched_pairs %>%
  rename(
    factor_ref = ReferenceFactor,
    factor_scot = SCOTFactor,
    factor_correlation = Correlation
  ) %>%
  mutate(
    factor_ref = standardize_factor_name(factor_ref),
...
flatten_enrichment <- function(enrich_list) {
  df_list <- lapply(names(enrich_list), function(nm) {
    ...
    data.frame(
      ...
      view = view,
      sign = sign,
      factor = standardize_factor_name(factor),
      pathway = paths,
      stringsAsFactors = FALSE
    )
  })
...
matched_pathway_concordance <- expand.grid(
  view = views,
  sign = signs,
  stringsAsFactors = FALSE
) %>%
  ...
  rowwise() %>%
  mutate(
    scot_sign_aligned = case_when(
      factor_correlation < 0 & sign == "positive" ~ "negative",
...
matched_pathway_concordance_export <- matched_pathway_concordance %>%
  mutate(
    ref_paths = vapply(ref_paths, function(x) paste(x, collapse = "; "), character(1)),
    scot_paths = vapply(scot_paths, function(x) paste(x, collapse = "; "), character(1))
  )

write.csv(
  matched_pathway_concordance_export,
  file.path(outdir, "Figure4C_pathway_concordance_matched_factors.csv"),
  row.names = FALSE
)
...
df_heat <- matched_pathway_concordance %>%
  ...
  select(RowLabel, Factor, jaccard) %>%
  distinct()

df_heat$RowLabel <- factor(
  df_heat$RowLabel,
  levels = c(
    "Proteomics (+)",
    "Proteomics (-)",
...
p4C <- ggplot(df_heat, aes(x = Factor, y = RowLabel, fill = jaccard)) +
...
p4C_shift <- cowplot::ggdraw() +
  cowplot::draw_plot(p4C, x = -0.04, y = 0, width = 1.04, height = 1)
...
# -----------------------------
ggsave(
  filename = file.path(outdir, "Figure4_combined_pathway_recovery.svg"),
  plot = fig4,
  device = svglite,
  width = 15,
  height = 20
...
ggsave(file.path(outdir, "Figure4A_transcriptomics_combined_dotplot.svg"),
       p4A, device = svglite, width = 9, height = 10)

ggsave(file.path(outdir, "Figure4B_proteomics_combined_dotplot.svg"),
       p4B, device = svglite, width = 11, height = 10)

ggsave(file.path(outdir, "Figure4C_pathway_concordance_heatmap.svg"),
       p4C, device = svglite, width = 7, height = 4.5)
