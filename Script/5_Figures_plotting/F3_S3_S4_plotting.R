...
ref_model_path  <- file.path(base_dir, "MOFA/output/single_cell_trained_model_MOFAcomplied.hdf5")
scot_model_path <- file.path(base_dir, "SCOT_plus/MOFA_output/SCOTplus_trained_model_MOFAcomp.hdf5")

ref_rna_meta_path  <- file.path(base_dir, "MOFA/input/5_MOFA_complied_preprocessing/transcriptomics_metadata.csv")
ref_prot_meta_path <- file.path(base_dir, "MOFA/input/5_MOFA_complied_preprocessing/proteomics_metadata.csv")
scot_meta_path     <- file.path(base_dir, "SCOT_plus/AGW_results/aligned_metadata.csv")

outdir <- file.path(base_dir, "sc_vs_scot+/MOFA_complied/Figure3")
...
extract_factor_df <- function(model, model_label) {
  ...
  # Expect columns: sample, factor, value
  ...
  factor_df <- factor_df %>%
    ...
    mutate(
      Model = model_label,
      Factor = factor(factor, levels = paste0("Factor", seq_len(max(as.integer(gsub("Factor", "", unique(factor)))))))
    )
...
# -----------------------------
# Helper: build combined Factor 1 heatmap
# -----------------------------
build_factor1_heatmap <- function(ref_model, scot_model,
                                  matched_pairs,
  ...
  # Extract data matrices (features x samples)
  ...
  # Match SCOT factor to reference Factor1
  if (!"Factor1" %in% matched_pairs$ReferenceFactor) {
    stop("Reference Factor1 not found in matched_pairs.")
  }
  
  row_match <- matched_pairs %>%
    filter(ReferenceFactor == "Factor1") %>%
    slice(1)
  ...
  ref_idx  <- match("Factor1", colnames(W_ref))
  scot_idx <- match(scot_factor_name, colnames(W_scot))
  
  if (is.na(ref_idx) || is.na(scot_idx)) {
    stop("Could not match Factor1 indices for heatmap construction.")
  }
  
  # Choose top reference features by absolute loading, restricted to features present in both models
  ...
  w_ref_factor1 <- W_ref[common_features, ref_idx]
  top_ref <- names(sort(abs(w_ref_factor1), decreasing = TRUE))
  
  # Optionally force a feature into the panel
  ...
  # Sample ordering by factor score
  ...
  ref_factor_values  <- Z_ref[, "Factor1"]
  ...
  annotation_col <- rbind(
    data.frame(
      sample = paste0("Reference__", ann_ref$sample),
      Model = "Reference",
      CellLine = ann_ref$CellLine
    ),
    data.frame(
      sample = paste0("SCOTplus__", ann_scot$sample),
      Model = "SCOT+-aligned",
  ...
  annotation_colors <- list(
    Model = c("Reference" = "#7F7F7F", "SCOT+-aligned" = "#377EB8"),
...
# Force factor order across both models
all_factor_levels <- paste0("Factor", seq_len(max(
  as.integer(gsub("Factor", "", as.character(unique(factor_df$Factor))))
)))
...
factor_df$Model_CellLine <- factor(
  factor_df$Model_CellLine,
  levels = c("Reference_C10", "Reference_SVEC", "SCOT+-aligned_C10", "SCOT+-aligned_SVEC")
)

# -----------------------------
# Figure 3A: variance explained
# -----------------------------
var_df <- var_df %>%
  mutate(
    Model = factor(Model, levels = c("Reference", "SCOT+-aligned")),
    Factor = factor(Factor, levels = c("Factor1", "Factor2", "Factor3", "Factor4")),
    View = factor(View, levels = c("Transcriptomics", "Proteomics"))
  )

factor_pos <- data.frame(
  Factor = factor(c("Factor1", "Factor2", "Factor3", "Factor4"),
                  levels = c("Factor1", "Factor2", "Factor3", "Factor4")),
  x_ref = c(0.85, 1.85, 2.85, 3.85),
  x_scot = c(1.15, 2.15, 3.15, NA)
)
...
var_df <- var_df %>%
  ...
  mutate(
    x = dplyr::case_when(
      Model == "Reference" ~ .data$x_ref,
      Model == "SCOT+-aligned" ~ .data$x_scot
    )
  ) %>%
  filter(!is.na(x))

p3A <- ggplot(var_df, aes(x = x, y = R2, fill = View)) +
  geom_col(width = 0.22) +
...
ggsave(filename = file.path(outdir, "Figure3A_variance_explained.svg"),
       plot = p3A, device = svglite, width = 5, height = 4.8)

write.csv(var_df, file.path(outdir, "variance_explained.csv"))

# -----------------------------
# Figure 3B: combined factor dot plot
# -----------------------------
cellline_colors <- c(
  "C10"  = "#000000",
  "SVEC" = "#D55E00"
...
factor_pos_B <- data.frame(
  Factor = factor(c("Factor1", "Factor2", "Factor3", "Factor4"),
                  levels = c("Factor1", "Factor2", "Factor3", "Factor4")),
  x_ref  = c(0.92, 1.42, 1.92, 2.42),
  x_scot = c(1.08, 1.58, 2.08, NA)
)
...
ggsave(
  filename = file.path(outdir, "Figure3B_factor_values.svg"),
  plot = p3B, device = svglite, width = 5, height = 4
)

write.csv(factor_df, file.path(outdir, "factor_values.csv"))

# -----------------------------
# Figure 3C: concordance heatmaps
# -----------------------------
...
# Sample-level factor correlation
cor_mat <- cor(
  Z_ref_aligned, Z_scot_aligned,
...
if (n_ref <= n_scot) {
  ...
  matched_pairs <- data.frame(
    ReferenceFactor = rownames(cor_mat),
  ...
  matched_pairs <- data.frame(
    ReferenceFactor = rownames(cor_mat)[assignment_t],
...
write.csv(cor_mat_ordered, file.path(outdir, "Figure3C_sample_level_concordance.csv"))
write.csv(matched_pairs, file.path(outdir, "Figure3_matched_factor_pairs.csv"), row.names = FALSE)

breaks <- seq(-1, 1, length.out = 101)
colors <- colorRampPalette(c("blue", "white", "red"))(100)

ph3C <- pheatmap::pheatmap(
  cor_mat_ordered,
  cluster_rows = TRUE,
  cluster_cols = FALSE,
  color = colors,
...
save_pheatmap_svg(ph3C, file.path(outdir, "Figure3C_sample_level_concordance.svg"),
                  width = 5, height = 4)

# -----------------------------
# Figure 3D: signed feature-level concordance heatmap
# -----------------------------
...
loading_mat_signed <- cbind(
  Transcriptomics = rna_corr_signed,
  Proteomics = prot_corr_signed
)
rownames(loading_mat_signed) <- matched_pairs$ReferenceFactor

write.csv(loading_mat_signed, file.path(outdir, "Figure3D_feature_level_concordance_signed.csv"))

ph3D <- pheatmap::pheatmap(
  loading_mat_signed,
  cluster_rows = TRUE,
  cluster_cols = FALSE,
  color = colors,
...
save_pheatmap_svg(ph3D, file.path(outdir, "Figure3D_feature_level_concordance_signed.svg"),
                  width = 5, height = 4)

# -----------------------------
# Figure 3E-F: Factor 1 heatmaps
# -----------------------------

ph3E_rna <- build_factor1_heatmap(ref_model = ref_model, 
                                  scot_model = scot_model, matched_pairs = matched_pairs,
                                  view_name = "Transcriptomics",
                                  nfeatures = 20,  force_feature = "H2-K1_rna")

ph3E_prot <- build_factor1_heatmap(ref_model = ref_model,
                                   scot_model = scot_model, matched_pairs = matched_pairs,
                                   view_name = "Proteomics", nfeatures = 20,
                                   force_feature = "H2-K1_prot")

ph3E_rna_noleg <- remove_pheatmap_legend(ph3E_rna)
ph3E_rna_noleg$gtable$grobs[[which(ph3E_rna_noleg$gtable$layout$name == "legend")]] <- grid::nullGrob()

g3E_rna  <- ggplotify::as.ggplot(ph3E_rna_noleg$gtable)
g3E_prot <- ggplotify::as.ggplot(ph3E_prot$gtable)

g3E_rna_tight <- g3E_rna + 
  theme(plot.margin = margin(t = 5, r = -0, b = 5, l = 30, unit = "pt"))

g3E_prot_shift <- cowplot::ggdraw() +
  cowplot::draw_plot(g3E_prot, x = -0.40, y = 0, width = 1.45, height = 1)

fig3E_combined <- cowplot::plot_grid(
  g3E_rna_tight,
  g3E_prot_shift,
  ncol = 2,
  rel_widths = c(1.5, 1)
...
ggsave(
  filename = file.path(outdir, "Figure3E_Factor1_combined_heatmap.svg"),
  plot = fig3E_combined, device = svglite, width = 12, height = 5
)

# -----------------------------
# Combined Figure 3A-F
# -----------------------------
...
g3A <- p3A
g3B <- p3B
g3C <- unclip_plot(ggplotify::as.ggplot(ph3C$gtable))
g3D <- unclip_plot(ggplotify::as.ggplot(ph3D$gtable))
g3E <- unclip_plot(fig3E_combined)
...
fig3_top_u <- unclip_plot(fig3_top)
fig3_mid_u <- unclip_plot(fig3_mid)
g3E_u      <- unclip_plot(g3E)
...
fig3_all <- cowplot::plot_grid(
  ...
  label_y = c(0.98, 0.98, 0.98, 0.98, 0.98),
  ncol = 1,
  rel_heights = c(1, 0.12, 1, 0.12, 1.1)
)

ggsave(
  filename = file.path(outdir, "Figure3_combined_A_to_E.svg"),
  plot = fig3_all, device = svglite, width = 12, height = 11
)

# -----------------------------
# Supplementary Figure 3: joint UMAP on shared latent factors
# -----------------------------

# Extract factors in long format
...
# Optional sign flips can be added here if needed later
...
pS3 <- ggplot(joint_plot_df, aes(x = UMAP1, y = UMAP2, colour = CellLine, shape = Model)) +
...
ggsave(
  filename = file.path(outdir, "Supplementary_Figure3_joint_UMAP.svg"),
  plot = pS3, device = svglite, width = 5, height = 4
)

# -----------------------------
# Supplementary Figure 4: combined signed Venn diagrams
# Clean GPB-style version
# -----------------------------
supp4_outdir <- file.path(outdir, "Supplementary_Figure4_Venn")
dir.create(supp4_outdir, recursive = TRUE, showWarnings = FALSE)

matched_pairs_s4 <- matched_pairs[seq_len(min(3, nrow(matched_pairs))), , drop = FALSE]
...
# NOTE:
# ggVennDiagram arranges the two-set Venn vertically here:
# top circle = Reference
# bottom circle = SCOT+
# =========================================================
...
build_view_block <- function(view_name, matched_pairs_df, ref_model, scot_model, n_top = 20) {
  ...
  for (row_idx in seq_along(signs)) {
    ...
    for (col_idx in seq_len(nrow(matched_pairs_df))) {
      ...
      ref_feats <- get_top_features_signed(
        model = ref_model,
        view = view_name,
        factor_name = ref_factor,
        sign = sign_i,
        n_top = n_top
      )
      
      scot_feats <- get_top_features_signed(
        model = scot_model,
        view = view_name,
        factor_name = scot_factor,
        sign = sign_i,
        n_top = n_top
      )
      ...
      panel_store[[plot_idx]] <- make_small_venn_plot(
        ref_features = ref_feats,
        scot_features = scot_feats
      )
      
      summary_list_local[[length(summary_list_local) + 1]] <- data.frame(
        View = view_name,
        Sign = sign_i,
  ...
  list(
    plot = block,
    summary = bind_rows(summary_list_local)
  )
}
...
# =========================================================
tx_block <- build_view_block(
  view_name = "Transcriptomics",
  matched_pairs_df = matched_pairs_s4,
  ref_model = ref_model,
  scot_model = scot_model,
  n_top = n_top
)

prot_block <- build_view_block(
  view_name = "Proteomics",
  matched_pairs_df = matched_pairs_s4,
  ref_model = ref_model,
  scot_model = scot_model,
  n_top = n_top
...
# =========================================================
# Final combined plot
# No headers; structure explained in figure legend
# =========================================================
...
supp4_plot <- (
  patchwork::wrap_elements(tx_boxed) /
    patchwork::plot_spacer() /
    patchwork::wrap_elements(prot_boxed) /
    patchwork::wrap_elements(bottom_labels)
) + plot_layout(
  heights = c(1, 0.03, 1, 0.12)
)

ggsave(
  filename = file.path(supp4_outdir, "Supplementary_Figure4_combined_venn_signed.svg"),
  plot = supp4_plot,
  device = svglite,
  width = 5,
  height = 6.8
...
write.csv(
  summary_df,
  file.path(supp4_outdir, "Supplementary_Figure4_feature_overlap_venn_signed_summary.csv"),
  row.names = FALSE
)
