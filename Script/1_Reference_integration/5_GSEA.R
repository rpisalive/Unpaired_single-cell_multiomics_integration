...
outdir <- "C:/Users/49152/Downloads/Multi-omics/MOFA/output/graphs/Single_cell_MOFAcomplied/GSEA/"
...
if (!all(model@samples_metadata$sample %in% model_samples)) {
  stop("Sample metadata join failed: some model samples were not matched.")
}
...
build_dotplot_df <- function(enrichment, sign_label, factors_vec, top_n = 10) {
  ...
  for (fac in factors_vec) {
    ...
    # Keep negative sign explicit in the combined data frame
...
extract_factor_weights <- function(model, view, factor) {
...
for (v in views) {
  ...
  # Check for duplicated names after cleaning
  dup_feat <- duplicated(feat_clean)
  if (any(dup_feat)) {
    warning(
      "Duplicated feature names detected after cleaning in view: ", v,
      ". Keeping first occurrence only in enrichment matching may be necessary."
    )
  }
...
for (view in views) {
  ...
  p_dot <- ggplot(df_dot, aes(x = Factor, y = Pathway, size = Genes, color = Score)) +
    ...
    theme(axis.text.x = element_text(size = 8, angle = 45, hjust = 1),
          axis.text.y = element_text(size = 7), axis.title.x = element_text(size = 8, face = "bold"),
  ...
  # Per-sign/ per-factor detailed output
  for (sign in c("positive", "negative")) {
    ...
    for (fac in valid_factors) {
      ...
      # Export weights table for the factor
